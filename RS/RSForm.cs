...
namespace BSIDCertificates.RS
{
    ...
    {
        DataTable dtBQCT1RS28Readings = new DataTable();
        ...
        private void RSForm_Load(object sender, EventArgs e)
        {
            try
            {
                dtBQCT1RS28Readings = GetBQCT1RS28ReadingsDetails();
                dtDeviceDetails = GetBQCT1RS28Details();
                dtTempHumidity = GetTempHumidity();

                reportViewer1.LocalReport.DataSources.Clear();

                ReportDataSource dsBQCT1RS28Readings = new ReportDataSource("dsBQCT1RS28Readings", dtBQCT1RS28Readings);
                ReportDataSource dsBQCT1RS28Details = new ReportDataSource("dsBQCT1RS28Details", dtDeviceDetails);
                ReportDataSource dsTempHumidity = new ReportDataSource("dsTempHumidity", dtTempHumidity);

                this.reportViewer1.LocalReport.DataSources.Clear();

                this.reportViewer1.LocalReport.DataSources.Add(dsBQCT1RS28Readings);
                this.reportViewer1.LocalReport.DataSources.Add(dsBQCT1RS28Details);
        ...
        private DataTable GetBQCT1RS28ReadingsDetails()
        {
            try
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("select BQCTRS28_Resistances, BQCTRS28_MasterDisplay, " +
                        "BQCTRS28_BeforeActual, BQCTRS28_BeforeDifference, BQCTRS28_BeforeError, " +
                        "BQCTRS28_AfterActual, BQCTRS28_AfterDifference, BQCTRS28_AfterError " +
                        "FROM tblBQCTRS28ReadingsMaster " , con);
                    //cmd.ExecuteNonQuery();
                    ...
                    using (SqlDataAdapter sda = new SqlDataAdapter())
                    {
                        ...
                        using (BridgestoneCalDataSet dsBQCT1RS28Readings = new BridgestoneCalDataSet())
                        {
                            sda.Fill(dtBQCT1RS28Readings);
                            return dtBQCT1RS28Readings;
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show("Oops. Something went wrong while fetching the BQCT#1-RS-28-Readings data.");
                LogError _LE = new LogError();
                _LE.LogDetails(ex);
                return null;
            }
        }
        private DataTable GetBQCT1RS28Details()
        {
            try
            {
                string constr = @"Data Source=SG2NWPLS19SQL-v06.mssql.shr.prod.sin2.secureserver.net; Initial Catalog=BridgestoneCal; Persist Security Info=True;User ID=bsid_admin;Password=BSID@2023";
                using (SqlConnection con = new SqlConnection(constr))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Device_ControlID", "BQCT#1-RS-28");
                    cmd.Parameters.AddWithValue("@Inst_Name", "Digital Multimeter");
            ...
            {
                MessageBox.Show("Oops. Something went wrong while fetching the BQCT#1-RS-28-Details.");
                LogError _LE = new LogError();
                _LE.LogDetails(ex);
                return null;
            }
        ...
        private DataTable GetTempHumidity()
        {
            ...
            {
                MessageBox.Show("Oops. Something went wrong while fetching the BQCT#1-RS-28-TempHumidity Details.");
                LogError _LE = new LogError();
                _LE.LogDetails(ex);
                return null;
            }
